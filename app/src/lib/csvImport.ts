...
export function parseCsvPhrases(csv: string): CsvPhraseRow[] {
  ...
  if (fieldCounts.size === 1 && (rows[0].length === 2 || rows[0].length === 3)) {
    const hasCategory = rows[0].length === 3
    return rows
      .map((r) => ({
        english: unescapeFormula(r[0] ?? '').trim(),
        text: unescapeFormula(r[1] ?? '').trim(),
        category: hasCategory ? unescapeFormula(r[2] ?? '').trim() || null : null,
      }))
      .filter((r) => r.english !== '')
  }
