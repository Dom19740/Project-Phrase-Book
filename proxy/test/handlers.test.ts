...
test('CORS: echoes back the hosted web app custom domain (https://app.travelchatter.dpbcreative.com)', async () => {
  mock.method(deviceRateLimit, 'limit', async () => ({ success: true }))
  mock.method(ipRateLimit, 'limit', async () => ({ success: true }))
  mock.method(redisOps, 'get', async () => null)
  mock.method(redisOps, 'set', async () => 'OK')
  ...
  await translateHandler(
    makeReq({
      headers: { 'x-device-id': 'device-1234', origin: 'https://app.travelchatter.dpbcreative.com' },
      body: { english: 'Hello', targetLangs: ['vi'] },
    }),
    res,
  )

  assert.equal(state.headers['Access-Control-Allow-Origin'], 'https://app.travelchatter.dpbcreative.com')
})
